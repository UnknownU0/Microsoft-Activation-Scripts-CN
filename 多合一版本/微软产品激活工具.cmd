...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误：脚本可能存在LF换行问题，或者脚本末尾缺少空行。  
echo:
...
cls
color 07
title  微软产品激活脚本(MAS) %masver% （由Unknown汉化） 
...
echo 该项目仅支持Windows 7/8/8.1/10/11及其服务器等版本。  
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 无法在系统中找到powershell.exe。  
echo 正在取消...  
goto MASend
)
...
%nceline%
echo 检测到脚本从临时文件夹启动，  
echo 您很可能直接从压缩文件直接运行本脚本。  
echo:
echo 请先解压压缩文件，再从解压后的文件夹中启动脚本。  
goto MASend
)
)
...
%eline%
echo 您正在运行过时的MAS版本 %masver%
echo ________________________________________________
echo:
if not defined _MASunattended (
echo [1] 下载最新的MAS（英文原版）
echo [0] 仍然继续执行
echo:
call :_color %_Green% "请使用键盘输入菜单选项 [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
cls
color 07
title  微软产品激活 %masver%（由Unknown汉化） 
mode 76, 30
...
echo:               激活方式:   ^|  适用产品：        ^|   激活时长 
echo:
echo:           [1] HWID        ^|  Windows           ^|   永久激活 
echo:           [2] Ohook       ^|  Office            ^|   永久激活 
echo:           [3] KMS38       ^|  Windows           ^|   激活至2038年 
echo:           [4] 在线KMS     ^|  Windows / Office  ^|   激活180天 
...
echo:           [7] 附加功能 
echo:           [8] 帮助 
echo:           [0] 退出                         【U】（由Unknown汉化） 
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "请使用键盘输入菜单选项 [1,2,3,4,5,6,7,8,0] :"
choice /C:12345678U0 /N
set _erl=%errorlevel%

if %_erl%==10 exit /b
if %_erl%==9 start https://github.com/UnknownU0/Microsoft-Activation-Scripts-CN & goto :MainMenu
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "请使用键盘输入菜单选项 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
cls
title  提取 $OEM$ 文件夹（由Unknown汉化） 
mode 76, 30

if not exist "!_desktop_!\" (
%eline%
echo 未检测到桌面文件夹位置，终止操作...  
echo _____________________________________________________
echo:
call :_color %_Yellow% "请任意按键后返回..."
pause >nul
goto Extras
)
...
echo:
call :_color2 %_White% "              [R] " %_Green% "打开网页查看介绍 "
echo:              [0] 返回  
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "请使用键盘输入菜单选项:"
...
if defined _error (
%eline%
echo 无法提取 $OEM$ 文件夹 至 桌面。  
) else (
echo:
call :_color %Blue% "%_oem%"
call :_color %Green% "$OEM$ 文件夹 已成功提取至 桌面。  "
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo 使用 KMS38 激活服务器 Cor/Acor 版本 ^(无 GUI 版本^)，  
echo 检查该项 %mas%oem-folder 
)
...
cls
color 07
title  HWID 激活 %masver%（由Unknown汉化） 
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回  "
set "_fixmsg=返回主菜单，选择故障排除并运行修复许可选项。  "
) else (
set "_exitmsg=退出  "
set "_fixmsg=在 MAS 文件夹中运行故障排除脚本，并选择修复许可选项。  "
)
...
echo 仅支持 Windows 10/11 的 HWID 激活。  
echo 请使用在线 KMS 激活选项。  
goto dk_done
)
...
%eline%
echo 不支持使用 HWID 激活 Windows Server。  
echo 请使用 KMS38 或在线 KMS 激活选项。  
goto dk_done
)
...
call :dk_color %Red% "无法启动 [sppsvc] 服务，剩余的过程可能需要很长时间..."
echo:
)
...
call :dk_color2 %_White% "     " %Gray% "不需要再次激活。  "
echo ___________________________________________________________________________________________
...
set error=1
call :dk_color %Red% "正在检查网络连接       【无法连接至互联网】  "
)
...
call :dk_color %Gray% "检查 Windows 更新 启用情况      【已停用】  "
if defined applist echo: %serv_e% | find /i "wuauserv" %nul% && (
call :dk_color %Blue% "Windows 更新无法正常工作。如果已禁用，请启用它。  "
reg query HKLM\SYSTEM\CurrentControlSet\Services\wuauserv /v WubLock %nul% && call :dk_color %Blue% "已使用 Sordum Windows Update Blocker 工具阻止了Windows更新。  "
)
)

reg query "HKLM\SOFTWARE\Policies\Microsoft\WindowsStore" /v DisableStoreApps %nul2% | find /i "0x1" %nul% && (
call :dk_color %Gray% "检查 Store App 启用情况      【已停用】  "
)
...
call :dk_color %Red% "检查 HWID 的备用版本     【%altedition% 激活 ID 未找到】  "
)

if not defined key (
%eline%
echo 【%winos% ^| %winbuild% ^| SKU:%osSKU%】  
echo 无法在支持的产品列表中找到此产品。  
echo 请确保您正在使用最新版本的脚本。  
echo %mas%
echo:
goto dk_done
)
...
call :dk_color %Blue% "[%altedition%] 版本的产品密钥将被用于HWID激活。  "
echo:
)
...
if !errorlevel! EQU 0 (
echo 更改 Windows 区域设置为美国     【成功】  
) else (
call :dk_color %Red% "更改 Windows 区域设置为美国     【失败】  "
)
)
...
call :dk_color %Red% "检查 ClipUp.exe 文件情况           【未找到，终止操作】  "
call :dk_color2 %Blue% "请访问此页面获取帮助：" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
call :dk_color %Red% "生成 GenuineTicket.xml          【失败，终止操作】  "
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装 GenuineTicket.xml      【由于ClipSVC 服务重启导致失败，正在等待...】  "
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查 ClipSVC 服务的 tokens.dat      【未找到】  "
)

%_xmlexist% (
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml      【clipup -v -o执行失败】  "
)

if exist "%ProgramData%\Microsoft\Windows\ClipSVC\Install\Migration\*.xml" (
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查票据迁移      【失败】  "
)
...
if defined _perm (
echo:
call :dk_color %Green% "%winos% 已通过数字许可证永久激活。  "
goto :dl_final
)
...
set error=1
echo:
call :dk_color %Red% "检查许可服务器      【连接失败】  "
...
set error=1
echo:
call :dk_color %Red% "检查网络问题                【已找到】 %error_code%"
call :dk_color2 %Blue% "查看此页面以获取帮助：" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
call :dk_color %Blue% "当您看到本消息，意味着此产品不支持硬件标识 (HWID) 激活。  "
call :dk_color %Blue% "请选择使用 KMS38 激活选项。  "
) else (
...
if !errorlevel! EQU 0 (
echo 恢复 Windows 区域设置           【成功】  
) else (
call :dk_color %Red% "恢复 Windows 区域设置           【失败】 [%name% - %nation%]  "
)
)

if %osSKU%==175 call :dk_color %Red% "%winos% 不支持在非 Azure 平台上激活。"
...
set error=1
call :dk_color %Red% "启用已禁用的服务              【成功】 [%serv_cste%]  "
)
...
call :dk_color %Blue% "重新启动系统以修复禁用服务错误 1058。  "
set showfix=1
)
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查系统引导方式            " %Blue% "【当前在安全模式中。请正常启动后再运行。】  "
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查审核模式         " %Blue% "【IMAGE_STATE_COMPLETE 状态未找到。请正常启动后再运行。】  "
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查 WinPE 系统                       " %Blue% "【当前在WinPE中。请正常启动后再运行。】  "
)


%psc% $ExecutionContext.SessionState.LanguageMode %nul2% | find /i "Full" %nul1% || (
set error=1
call :dk_color %Red% "检查 Powershell 状态                【未响应】  "
)
...
if %dism_error% NEQ 0 (
call :dk_color %Red% "检查 DISM 状态                【未响应】  %dism_error%"
)


if not defined officeact if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
set error=1
call :dk_color %Red% "检查评估包  【在评估版 Windows 中安装了非评估许可证】  "
if /i %dism_error%==[0x800F0805] (
for %%# in (4 125 126 188 191 205) do if "%osSKU%"=="%%#" (
call :dk_color %Blue% "评估版 Windows 无法激活，安装不同的许可证可能会导致错误。  "
call :dk_color %Blue% "建议安装完整版本的 %winos%。  "
call :dk_color %Blue% "你可以从这里下载： %mas%genuine-installation-media.html"
set showfix=1
)
)
)
...
call :dk_color %Gray% "检查 WMIC.exe 状态                    【未找到】  "
)
...
set error=1
call :dk_color %Red% "检查 WMI 状态                       【未响应】  "
call :dk_color %Blue% "在 MAS 中，转到故障排除并运行修复 WMI 选项。  "
set showfix=1
)
...
call :dk_color %Red% "检查 SLC/WMI/REG SKU                【找到差异 - SLC:%slcSKU% WMI:%wmiSKU% Reg:%regSKU%】  "
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查 Rearm 状态                       " %Blue% "【需要重新启动系统】  "
)
...
set error=1
set showfix=1
call :dk_color2 %Red% "检查 ClipSVC 状态                     " %Blue% "【需要重新启动系统】  "
)


for /f "skip=2 tokens=2*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform" /v "SkipRearm" %nul6%') do if /i %%b NEQ 0x0 (
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform" /v "SkipRearm" /t REG_DWORD /d "0" /f %nul%
call :dk_color %Red% "检查 SkipRearm 状态                 【未找到默认值 0。正在更改为 0。】  "
...
call :dk_color %Red% "检查 SPP 注册表键值               【找到错误的模块】  "
call :dk_color %Blue% "可能是由游戏欺骗器引起的。查看帮助： %mas%troubleshoot"
set error=1
set showfix=1
)
...
%psc% "!d!" %nul%
call :dk_color %Gray% "检查 SPP Token 文件夹               【未找到，已执行创建。】 [%tokenstore%\]"
)
...
if not defined applist (
set error=1
call :dk_color %Red% "检查激活 IDs                【未找到】  "
)
)
...
set error=1
set showfix=1
call :dk_color %Red% "检查 sppsvc.exe 文件                【未找到】  "
)
...
if defined wpaerror call :dk_color %Red% "检查 WPA 注册表键值 【发现错误】 [注册表计数-!count!]  "
)
...
if defined permerror call :dk_color %Red% "检查 SPP 权限                【找到错误】"

set showfix=1
call :dk_color %Blue% "%_fixmsg%"
if not defined permerror call :dk_color %Blue% "如果激活仍然失败，请运行修复 WPA 注册表选项。  "
)
...
call :dk_color %_Yellow% "请按任意键 %_exitmsg%..."
pause %nul1%
exit /b
...
cls
color 07
title  Ohook 激活 %masver% （由Unknown汉化） 
...
%eline%
echo 检测到不支持的操作系统版本 【%winbuild%】。  
echo Ohook 激活支持 Windows 8 及更高版本以及它们的服务器等效版本。  
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "请使用键盘输入菜单选项 [1,2,3,0]"
choice /C:1230 /N
set _el=!errorlevel!
if !_el!==4  exit /b
...
call :dk_color %Red% "无法启动 [sppsvc] 服务，其余的过程可能需要很长时间...  "
echo:
)
...
echo:
call :dk_color %Red% "检查到不受支持的 Office 安装     【 %o14msi%%o14c2r%%o16uwp%】  "
)
...
call :dk_color %Red% "检查 ClickToRun 服务             【未找到，找到 Office 16.0 相关文件】  "
set o16c2r=
set error=1
)
...
call :dk_color %Red% "检查 ClickToRun 服务             【未找到，找到 Office 15.0 相关文件】  "
set o15c2r=
set error=1
)
...
echo:
if not "%o14msi%%o14c2r%%o16uwp%"=="" (
call :dk_color %Red% "检查已安装支持的 Office 版本       【未找到】  "
) else (
...
echo:
call :dk_color %Blue% "请从以下网址下载并安装Office，然后再尝试。  "
echo:
echo %mas%genuine-installation-media.html
goto dk_done
)
...
call :dk_color %Gray% "检查Office多版本安装情况        【发现。最好只安装一个版本】  "
)
...
call :dk_color %Red% "检查已安装的产品         【未找到产品ID。激活中止...】  "
set error=1
goto :starto16c2r
)
...
if not defined _oIds (
call :dk_color %Red% "检查已安装产品              【未找到产品ID。激活中止...】  "
set error=1
goto :startmsi
)
...
if not defined _sid (
call :dk_color %Red% "检查用户账户SID              【未找到】  "
)
...
if defined sub_next echo 移除Office vNext阻止程序             【成功】  
...
if %upk_result%==1 echo 卸载其他或宽限密钥           【成功】  
if %upk_result%==2 call :dk_color %Red% "卸载其他或宽限密钥           【失败】  "
...
cls
mode 99, 28
title  卸载 Ohook 激活 %masver% （由Unknown汉化） 
...
if defined _unerror (
call :dk_color %Red% "无法卸载Ohook激活。  "
call :dk_color %Blue% "如果Office应用程序正在运行，请关闭它们，然后重试。  "
) else (
call :dk_color %Green% "成功卸载Ohook激活。  "
)
)
echo __________________________________________________________________________________________
...
call :dk_color %Red% "安装通用产品密钥        [%_key%] [%_prod%]  【失败】 %errorcode%  "
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
call :dk_color %Gray% "安装缺失的许可证文件        [Office %oVer%.0 %_prod%]  【成功】  "
) || (
set error=1
...
echo 将系统的sppc.dll创建符号链接到        ["%_hookPath%\sppcs.dll"]  【成功】  
echo 提取自定义 %_hook% 到        ["%_hookPath%\sppc.dll"]  【成功】  
) else (
set error=1
call :dk_color %Red% "将系统的sppc.dll创建符号链接         【失败】  "
call :dk_color %Red% "提取自定义 %_hook%           【失败】  "
echo ["%_hookPath%\sppc.dll"]
echo:
call :dk_color %Blue% "请关闭包括Outlook在内的所有Office应用程序，然后重试。  "
call :dk_color %Blue% "如果问题仍未解决，请重新启动系统，然后重试。  "
)
...
set error=1
call :dk_color %Red% "检查脚本中的产品              【在脚本中未找到 Office %oVer%.0 %%#】  "
echo 请确保您正在使用最新的MAS脚本。  
)
)
...
if not defined _oIds (
set error=1
call :dk_color %Red% "检查已安装的产品             【未找到产品ID。中止激活操作...】  "
exit /b
)
...
cls
color 07
title  KMS38 激活 %masver% （由Unknown汉化） 
...
%eline%
echo 检测到不支持的操作系统版本 【%winbuild%】。  
echo KMS38激活支持Windows 10/11/Server，版本号为14393及以上。  
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "请使用键盘输入菜单选项 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo:
call :dk_color2 %_White% "     " %Green% "正在检查：%winos% 已永久激活。  "
call :dk_color2 %_White% "     " %Gray% "无需激活。  "
echo ___________________________________________________________________________________________
if %_unattended%==1 goto dk_done
echo:
choice /C:10 /N /M ">    [1] 激活 [0] %_exitmsg% : "
...
echo 无法激活服务器评估版。请将其转换为完整的服务器操作系统。  
echo:
echo 在MAS中，转到“附加功能”并使用“更改版本”选项。  
) else (
echo 评估版无法激活。  
...
%eline%
echo 在 Server Core/Azure Core（无 GUI）版本中，clipup.exe 不存在。  
echo clipup.exe 是 KMS38 激活所必需的。  
echo 请查看以下页面以了解如何激活：  
echo %mas%kms38.html
goto dk_done
)
...
for /f "skip=2 tokens=2*" %%a in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v PROCESSOR_ARCHITECTURE') do set arch=%%b
for /f "tokens=6-7 delims=[]. " %%i in ('ver') do if "%%j"=="" (set fullbuild=%%i) else (set fullbuild=%%i.%%j)
echo 检查系统信息                     [%winos% ^| %fullbuild% ^| %arch%]  
...
call :dk_color %Blue% "要进行 IotEnterpriseS KMS38 激活，Windows 必须更新到 19044.2788 或更高版本。  "
)
)
...
echo:
if defined changekey (
call :dk_color %Blue% "[%altedition%] 将使用版本产品密钥启用 KMS38 激活。  "
echo:
)
...
call echo 正在检查已安装的产品密钥            [Partial Key - %%_partial%%] [Volume:GVLK]  
)
...
if !error_code! EQU 0 (
call :dk_refresh
echo 正在安装 KMS 客户端设置密钥      [%key%] 【成功】   
) else (
call :dk_color %Red% "正在安装 KMS 客户端设置密钥      [%key%] 【失败】 !error_code!  "
if not defined error (
call :dk_color %Blue% "%_fixmsg%"
set showfix=1
)
...
if not defined app (
call :dk_color %Red% "正在检查已安装的 GVLK 激活 ID   【未找到】 操作中止...  "
call :dk_color2 %Blue% "请查看此页面以获取帮助：" %_Yellow% " %mas%troubleshoot"
goto :dk_done
)
...
echo 正在添加特定的 KMS 主机          [localhost 127.0.0.2] 【成功】  
) else (
call :dk_color %Red% "正在添加特定的 KMS 主机            [localhost 127.0.0.2] 【失败】  "
)
...
call :dk_color %Red% "复制 clipup.exe 文件到              [%systemroot%\System32\] 【失败】 已中止...  "
goto :k_final
)
)
...
if not exist "%tdir%\GenuineTicket.xml" (
call :dk_color %Red% "生成 GenuineTicket.xml 文件  【失败，中止进程】  "
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :k_final
) else (
echo 生成 GenuineTicket.xml           【成功】  
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装 GenuineTicket.xml            【失败，需要重新启动 ClipSVC 服务，请稍候…】  "
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            【失败，使用 clipup -v -o 命令】  "
)
...
if %errorlevel%==0 (
echo 应用 SKU-ID 重装               【成功】  
) else (
call :dk_color %Red% "应用 SKU-ID 重装               【失败】  "
)
call :dk_refresh
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
echo 从 KMS 保护 KMS38                【成功】 【锁定了一个注册表键】  
) || (
call :dk_color %Red% "从 KMS 保护 KMS38                【失败】 【未成功锁定注册表键】  "
)
)
...
call :dk_color %Red% "%winos% 不支持在非 Azure 平台上进行激活。  "
)
...
cls
mode 99, 28
title  移除 KMS38 保护 %masver% （由Unknown汉化） 
...
echo:
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "移除特定的 KMS 主机             【失败】  "
) || (
echo 移除特定的 KMS 主机             【成功】  
)
...
cls
color 07
title  在线 KMS 激活 %masver% （由Unknown汉化） 
...
if %winbuild% GEQ 22621 echo 请确保在可选功能中启用了 WMIC。  
goto Done
)
...
echo 系统中的 WMI 无响应。  
echo:
echo 在 MAS（管理和自动化解决方案）中，转到故障排除并运行修复 WMI 选项。  
goto Done
)
...
set "_title=在线 KMS 激活 %masver% （由Unknown汉化） "
set _gui=
...
if defined _oldtsk       call :_color2 %_White% "              [4] 安装自动续订        " %_Red% "【旧版已安装】  "
if not defined _tskinstalled if not defined _oldtsk echo.              [4] 安装自动续订  
echo.              [5] 卸载  
echo.              _______________________________________________  
echo.
if %_Debug%==0 (
echo.              [6] 开启 Debug 模式  [No]  
) else (
call :_color2 %_White% "              [6] 开启 Debug 模式  " %_Red% "[Yes]  "
)
if %vNextOverride% EQU 1 (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆盖安装 Office vNext  " %_Red% "[Yes]  "
) else (
echo               [7] 覆盖安装 Office vNext  [Yes]  
)
) else (
if %sub_next% EQU 1 (
call :_color2 %_White% "              [7] 覆盖安装 Office vNext  " %_Yellow% "[No]  "
) else (
echo               [7] 覆盖安装 Office vNext  [No]  
)
)
echo.              _______________________________________________       
echo.
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "请使用键盘输入菜单选项 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
if %_Debug% EQU 0 (
  ...
  if %Silent% EQU 0 (
  echo.
  echo 当前在 Debug 模式中运行...  
  if not defined _args (echo 日志收集完成后本窗口将自动关闭  ) else (echo 请稍等...  )
  echo.
  echo 写入 debug 日志到：  
  ...
  @echo on
  @prompt $G
  @call :Begin >"!_log!_tmp.log" 2>&1 &cmd /u /c chcp 936&type "!_log!_tmp.log">"!_log!_Debug.log"&del "!_log!_tmp.log"
)
@echo off
...
if defined _unattended (
echo.
call :_color %_Red% "互联网未连接，仍然继续进行该过程。  "
) else (
%eline%
echo 互联网未连接。  
echo:
...
if %W1nd0ws% EQU 1 if %ActWindows% EQU 0 (echo.&echo Windows激活已经关闭...  )
...
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 检查中：  %%x
echo 产品已通过 KMS 激活至 2038 年。  
set _keepkms38=1
exit /b
)
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %act_attempt% LSS 1 (
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 激活中： %%x  
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 激活中： %%x  
)
...
if %OSType% EQU Win7 echo 由于不合格的 OEM BIOS，无法在此计算机上对 Windows 7 进行 KMS 激活。  
echo 请查看详细的 "Read Me"（自述文件）获取更多信息。  
exit /b
)
if %act_attempt% LSS 1 if %ERRORCODE% EQU -1073417728 (
echo 产品激活失败： 0xC004F200  
echo Windows 需要重新构建与激活相关的文件。  
echo 请查看详细的 补丁 KB2736303 获取更多信息。  
exit /b
)
if %act_attempt% LSS 1 if %ERRORCODE% EQU -1073422315 (
echo 产品激活失败： 0xC004E015  
echo 运行 slmgr.vbs /rilc 命令以进行修复。  
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 错误：未检测到 Office C2R 产品ID。  &goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_LicensesPath%\ProPlus*.xrm-ms" (
if %_Office15% EQU 0 (echo 错误：未检测到 Office C2R 授权文件。  &goto :%_fC2R%) else (goto :Reg15istry)
)
if not exist "%_Integrator%" (
if %_Office15% EQU 0 (echo 错误：未检测到 Office C2R 许可证集成工具。  &goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 错误：未检测到 Office 2013 C2R 产品ID。  &goto :%_fC2R%) else (goto :CheckC2R)
)
if not exist "%_Licenses15Path%\ProPlus*.xrm-ms" (
if %_Office16% EQU 0 (echo 错误：未检测到 Office 2013 C2R 授权文件。  &goto :%_fC2R%) else (goto :CheckC2R)
)
if %winbuild% LSS 9200 if not exist "%_OSPP15VBS%" (
if %_Office16% EQU 0 (echo 错误：未检测到 Office 2013 C2R 许可证工具 OSPP.vbs。  &goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo.
echo Microsoft Office 使用 vNext 许可证已激活。  
)
if %sub_proj% EQU 1 (
  for %%a in (%_PrjSKU%) do set _%%a=0
echo.
echo Microsoft Project 使用 vNext 许可证已激活。  
)
if %sub_vsio% EQU 1 (
  for %%a in (%_VisSKU%) do set _%%a=0
echo.
echo Microsoft Visio 使用 vNext 许可证已激活。  
)
...
set _O365ProPlus=1
echo O365HomePrem 2016 套件 ^<-^> Mondo 2016 许可证  
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 (
  set _Standard2021=1
  echo %%a 2021 套件 -^> Standard 2021 许可证  
  call :InsLic Standard2021
  )
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 if !_%%a2021! EQU 0 (
  set _Standard2019=1
  echo %%a 2019 套件 -^> Standard%_ons% 许可证  
  call :InsLic Standard%_tag%
  )
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 if !_%%a2021! EQU 0 if !_%%a2019! EQU 0 (
  set _Standard=1
  echo %%a 2016 套件 -^> Standard%_ons% 许可证  
  call :InsLic Standard%_tag%
  )
)
...
if !_O365ProPlus! EQU 0 if !_ProPlus! EQU 0 if !_Professional! EQU 0 if !_Standard! EQU 0 (
  set _Standard=1
  echo %%a 2013 套件 -^> Standard 2013 许可证  
  call :Ins15Lic Standard
  )
)
...
if not [%Act_OK%]==[1] (
echo.
echo 如果出现任何问题，请查看：https://mass%-%grave.dev/troubleshoot  
)
...
call :_color %Red% "您的互联网连接中阻止了端口 1688。  "
echo.
echo 原因：    可能连接了受限制的互联网（如学校或公司网络），  
echo           或防火墙阻止了连接。  
echo.
echo 解决方案：要么使用另一个互联网连接，要么使用离线 KMS。  
echo           https://github.com/abbodi1406/KMS_VL_ALL_AIO  
) else (
echo 端口 1688 连接测试已通过。  
echo.
echo 请确保系统文件没有被防火墙阻止。  
echo 如果问题仍然存在，请尝试使用离线 KMS。  
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO  
)
echo.
)
...
set error_=9
echo 无法完全清除 KMS 缓存。  
reg query "HKLM\%SPPk%\%_wApp%" /s 2>nul | findstr /i "127.0.0.2" >nul && echo KMS38激活已被锁定。  
) || (
echo 已成功清除 KMS 缓存。  
)
exit /b
...
title 完全卸载在线 KMS 激活 %masver% （由Unknown汉化） 
...
echo ## 注意 ##  
echo.
echo 为了确保 Office 程序不显示非正版提示，  
echo 请运行一次激活选项，之后不要再卸载。  
echo __________________________________________________________________________________________
)
...
echo __________________________________________________________________________________________
echo.
call :_color %Green% "成功完成了在线 KMS 的完全卸载。  "
echo __________________________________________________________________________________________
)
...
echo 删除 [任务计划] Online_KMS_Activation_Script-Run_Once  
...
cls
mode con cols=91 lines=30
title  安装自动续订激活服务 %masver% （由Unknown汉化） 
...
call :createInfo.txt
%psc% "$f=[io.file]::ReadAllText('!_batp!') -split \":_extracttask\:.*`r`n\"; [io.file]::WriteAllText('%_dest%\Activation_task.cmd', '@REM Dummy ' + '%random%' + [Environment]::NewLine + $f[1].Trim(), [System.Text.Encoding]::UTF8);"
title  安装自动续订激活服务 %masver% （由Unknown汉化） 
...
echo.
echo 提示：  
echo 如果检测到互联网连接，将每周自动续订激活。  
echo 它只会续订已安装的 KMS 许可证，不会将任何许可证转换为 KMS。  
echo __________________________________________________________________________________________
echo.
if defined ActTask (
call :_color %Green% "成功创建了续订和激活任务。  "
) else (
call :_color %Green% "成功创建了续订任务。  "
)
echo.
call :_color %Gray% "请确保至少运行过一次激活选项。  "
echo __________________________________________________________________________________________
)
...
(
echo   这个脚本的用途是使用在线 KMS 续订您的 Windows/Office KMS 许可证。  
echo:
echo   如果已创建续订/激活计划任务，则会存在以下内容，  
echo:
echo   - 计划任务  
echo     Activation-Renewal    【续订/每周】  
echo     Activation-Run_Once   【激活任务-激活后自动删除】  
echo     只有在系统连接到互联网时，计划任务才会运行。  
echo:
echo   - 文件  
...
:_extracttask:
@echo off
chcp 936 > %temp%\Unknown_MAS_AIO_CHCP_TEMP_3ITFG6ZQ
...
if defined run_once (
echo.
echo 删除计划任务 Activation-Run_Once  
schtasks /delete /tn Activation-Run_Once /f %nul%
)
...
if %_wmic% EQU 1 for /f "tokens=2 delims==" %%x in ('"wmic path !_path! where ID='!_actid!' get Name /VALUE" 2^>nul') do call echo 激活中： %%x  
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%x in ('%psc% "(([WMISEARCHER]'SELECT Name FROM !_path! WHERE ID=''!_actid!''').Get()).Name | %% {echo ('Name='+$_)}" 2^>nul') do call echo 激活中： %%x  
exit /b
...
echo 产品激活失败： 0xC004F035  
if [%1]==[act_win] if %winbuild% LSS 9200 echo 由于不合格的 OEM BIOS，此计算机无法使用 KMS 激活 Windows 7。  
exit /b
)
...
cscript //nologo slmgr.vbs /dli || (echo 未成功执行 slmgr.vbs&del /f /q slmgr.vbs&popd&goto :casVend)
...
echo %line2%
echo.
powershell "write-host -back 'Black' -fore 'Yellow' '已安装永久激活的 Office（Ohook）。  '; write-host -back 'Black' -fore 'Yellow' '您可以忽略下面的 Office 激活状态。  '"
echo.
)
...
if %_cwmi% EQU 0 (
echo:
echo 错误：系统中的 WMI 未响应。  
echo:
echo 在 MAS 中，转到故障排除并运行修复 WMI 选项。  
echo:
echo 请任意按键后返回...  
pause >nul
exit /b
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=[+]该计算机已永久激活。  ") else (set "ExpireMsg=[+]该产品已永久激活。  ")
  ) else (
  set "LicenseMsg=%_mTag% 激活到期时间：%GracePeriodRemaining% 分钟（%_gpr% 天）  "
  if defined _xpr set "ExpireMsg=%_mTag% 激活将于 %_xpr% 到期  "
  )
)
if %LicenseStatus% EQU 2 (
set "License=Initial grace period"
if defined _xpr set "ExpireMsg=初始宽限期结束于 %_xpr%  "
)
if %LicenseStatus% EQU 3 (
set "License=Additional grace period (KMS license expired or hardware out of tolerance)"
if defined _xpr set "ExpireMsg=额外宽限期结束于 %_xpr%  "
)
if %LicenseStatus% EQU 4 (
set "License=Non-genuine grace period."
if defined _xpr set "ExpireMsg=非正版宽限期结束于 %_xpr%  "
)
if %LicenseStatus% EQU 6 (
set "License=Extended grace period"
if defined _xpr set "ExpireMsg=延长宽限期结束于 %_xpr%  "
)
...
set "KmsDns=来自 DNS 的 KMS 机器名称： %DiscoveredKeyManagementServiceMachineName%:%DiscoveredKeyManagementServiceMachinePort%"
...
if defined ProductKeyChannel echo 密钥通道： %ProductKeyChannel%  
...
%eline%
echo 检测到不受支持的操作系统版本。  
echo 该命令仅适用于 Windows 8/8.1/10/11 及其服务器等同版本。  
goto :at_back
)
...
echo %line%
echo:
echo      Dism 使用 Windows 更新提供修复所需的文件。  
echo      这将需要 5-15 分钟或更长时间。  
echo %line%
echo:
echo      注意事项：  
echo:
call :_color2 %_White% "     - " %Gray% "确保已连接到互联网。  "
...
echo:
call :_color %Gray% "CBS 和 DISM 日志将被复制到桌面上的 AT_Logs 文件夹中。  "
goto :at_back
...
echo:
echo      如果 SFC 无法修复某些内容，请再次运行命令，看看下一次是否能够修复。   
echo      有时可能需要运行 sfc /scannow 命令 3 次，在每次之后重新启动计算机，  
echo      以完全修复所有可以修复的内容。  
echo:   
echo %line%
echo:
...
echo:
call :_color %Gray% "CBS 日志将被复制到桌面上的 AT_Logs 文件夹中。  "
goto :at_back
...
title  修复许可证 ^(ClipSVC ^+ Office vNext ^+ SPP ^+ OSPP^) （由Unknown汉化） 
...
echo:
echo       - 此选项将执行以下操作：   
echo            - 将 Windows 和 Office 激活置于非活动状态，您可能需要重新激活  
echo              如果 Windows 使用主板/OEM/数字许可证激活，则不必担心  
echo:
echo            - 清除 ClipSVC、Office vNext、SPP 和 OSPP 许可证  
echo            - 修复 tokens 文件夹和注册表中的 SPP 权限  
echo            - 触发 Office 的修复选项。  
echo:
call :_color2 %_White% "      - " %Red% "仅在必要时应用此选项。  "
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回   : "
...
if %winbuild% LSS 10240 (
echo ClipSVC 许可证重建仅支持 Win 10/11 和服务器等同版本。   
echo 跳过...  
goto :cleanvnext
)
...
echo:
echo 删除 Volatile 和 Protected 注册表键值...  
echo [%RegKey%]
reg query "%RegKey%" %nul% && (
call :_color %Red% "【执行失败】  "
echo 重新启动系统，这将自动删除该注册表键。  
) || (
echo 【执行成功】  
)
...
if not defined regfound echo 未找到 - Office vNext 注册表键  
...
echo %line%
echo:
call :_color %Blue% "重建 OSPP 许可证令牌  "
echo:
...
if /i "%arch%"=="ARM64" (
echo:
echo 检测到 ARM64 Windows。  
echo 您需要在 Windows 设置中使用修复选项修复 Office。  
echo:
...
echo:
call :_color %_Yellow% "一个窗口将会弹出，在那个窗口里你需要选择【快速(Quick)】修复选项......    "
...
if defined uwp16 (
echo:
echo 注意：跳过Office 16.0 UWP的修复。  
echo       你需要在Windows设置中使用重置选项进行操作。  
echo ________________________________________________________________
echo:
start ms-settings:appsfeatures
)
...
if defined c2r14 (
echo:
echo 注意：跳过Office 14.0 C2R的修复。  
echo       你需要在Windows设置中使用修复选项进行操作。  
echo ________________________________________________________________
echo:
start appwiz.cpl
)

if defined msi14_68 if exist "%msi14repair68%" echo 运行中 - "%msi14repair68%"                    & "%msi14repair68%"
if defined msi14_86 if exist "%msi14repair86%" echo 运行中 - "%msi14repair86%"                    & "%msi14repair86%"
if defined msi15_68 if exist "%msi15repair68%" echo 运行中 - "%msi15repair68%"                    & "%msi15repair68%"
if defined msi15_86 if exist "%msi15repair86%" echo 运行中 - "%msi15repair86%"                    & "%msi15repair86%"
if defined msi16_68 if exist "%msi16repair68%" echo 运行中 - "%msi16repair68%"                    & "%msi16repair68%"
if defined msi16_86 if exist "%msi16repair86%" echo 运行中 - "%msi16repair86%"                    & "%msi16repair86%"
if defined c2r15_68 if exist "%c2r15repair68%" echo 运行中 - "%c2r15repair68%" REPAIRUI RERUNMODE & "%c2r15repair68%" REPAIRUI RERUNMODE
if defined c2r15_86 if exist "%c2r15repair86%" echo 运行中 - "%c2r15repair86%" REPAIRUI RERUNMODE & "%c2r15repair86%" REPAIRUI RERUNMODE
if defined c2r16_68 if exist "%c2r16repair68%" echo 运行中 - "%c2r16repair68%" scenario=Repair    & "%c2r16repair68%" scenario=Repair
if defined c2r16_86 if exist "%c2r16repair86%" echo 运行中 - "%c2r16repair86%" scenario=Repair    & "%c2r16repair86%" scenario=Repair
...
call :_color %Red% "【执行失败】  "
echo:
call :_color %Blue% "建议选择【重启】选项，然后再次应用修复WMI选项。  "
echo %line%
echo:
choice /C:21 /N /M "> [1] 重启计算机  [2] 撤销更改：  "
...
echo:
echo 检查 WMI 状态  
call :_color %Green% "【正常工作中】  "
goto :at_back
)

echo:
echo 注册.dll文件并编译.mof和.mfl文件  
...
cls
color 07
title  更改Windows版本 %masver% （由Unknown汉化） 
...
DISM /English /Online /Get-CurrentEdition %nul%
cmd /c exit /b !errorlevel!
echo DISM命令执行失败 [错误代码 - 0x!=ExitCode!]  
echo 操作系统版本未正确检测到。正在取消...  
echo:
echo 请查看此页面以获取帮助：. %mas%troubleshoot
goto ced_done
)
...
%line%
echo:
if defined dismnotworking call :dk_color %Red% "DISM.exe未响应。  "
call :dk_color %Gray% "未找到目标版本。  "
echo 当前版本 [%osedition% ^| %winbuild%] 不能被更改为任何其他版本。  
%line%
goto ced_done
)
...
call :dk_color %_Yellow% "注意 - DISM.exe未响应。  "
if /i "%osedition:~0,4%"=="Core" call :dk_color %_Yellow% "     - 一旦更改为专业版，您将看到更多的版选项。  "
)
%line%
echo:
...
echo [0]  %_exitmsg%
echo:
call :dk_color %_Green% "在键盘上输入选项号码，然后按“Enter”键：  "
set /p inpt=
...
%line%
echo:
if defined dismnotworking call :dk_color %_Yellow% "DISM.exe未响应。  "
...
echo:
echo  - 继续之前请保存您的工作，系统将自动重启。  
echo:
echo  - 一旦更改了版本，您将需要使用HWID选项进行激活。  
%line%
echo:
...
if !error_code! EQU 0 (
call :dk_refresh
call :dk_color %Green% "【执行成功】  "
echo:
call :dk_color %Gray% "需要重新启动才能正确更改版本。  "
) else (
call :dk_color %Red% "【执行失败】   [错误代码： 0x!=ExitCode!]  "
echo 请查看此页面以获取帮助：. %mas%troubleshoot
)
)
...
echo:
if defined dismnotworking call :dk_color %_Yellow% "注意 - DISM.exe未响应。  "
echo 将当前版本 [%osedition%] %winbuild% 更改为 [%targetedition%]  
echo:
call :dk_color %Blue% "重要-在继续之前，请保存您的工作，系统将自动重新启动。  "
echo:
...
call :dk_color %Blue% "在此阶段，您必须重新启动系统。  "
